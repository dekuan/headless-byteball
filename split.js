...
function createSplitOutputs( address, asset, handleOutputs )
{
	...
	db.query
	(
		"SELECT amount FROM outputs WHERE address=? AND asset " + asset_value + " AND is_spent=0 ORDER BY amount DESC LIMIT 1",
		[ address ],
		function( rows )
		{
			if ( 0 === rows.length )
...
function checkAndSplitLargestOutput( address, asset )
{
	...
	db.query
	(
		//	see if the largest output is greater than the 1/10th of the total
...
function startCheckingAndSplittingLargestOutput( address, asset, period )
{
	...
	{
		//	asset omitted but period is set
		period	= asset;
		asset	= null;
	}
