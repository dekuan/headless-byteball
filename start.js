...
function readKeys( onDone )
{
	...
	fs.readFile( KEYS_FILENAME, 'utf8', function( err, data )
	{
		...
		if ( err )
		{
			...
			rl.question
			(
				"Please name this device [" + suggestedDeviceName + "]: ",
				function( deviceName )
				{
					if ( ! deviceName )
					{
...
function createWallet( xPrivKey, onDone )
{
	...
	{
		walletDefinedByKeys.issueNextAddress
		(
			wallet_id,
			0,
...
function readFirstAddress( handleAddress )
{
	db.query
	(
		"SELECT address FROM my_addresses WHERE wallet = ? AND address_index = 0 AND is_change = 0 ",
		[ wallet_id ],
		function( rows )
		{
			if ( 0 === rows.length )
...
function issueOrSelectNextMainAddress( handleAddress )
{
	...
	walletDefinedByKeys.issueOrSelectNextAddress
	(
		wallet_id,
		0,
		function( objAddr )
...
if ( conf.permanent_pairing_secret )
{
	...
	(
		"INSERT " + db.getIgnore() +
		" INTO pairing_secrets ( pairing_secret, is_permanent, expiry_date ) VALUES (?, 1, '2038-01-01')",
		...
		readSingleWallet( function( wallet )
		{
			...
			(
				...
				function( rows )
				{
					...
					//	we don't need any of its functions but it listens for hub/* messages
 ...
 */
exports.readSingleWallet			= readSingleWallet;			//	db : wallets
exports.readSingleAddress			= readSingleAddress;			//	db : my_addresses
exports.readFirstAddress			= readFirstAddress;			//	db : my_addresses
